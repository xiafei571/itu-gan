...
#    recipes = load_label(DATA_SET + '/label-for-' + attribute + '.json')
    ...
    for i in range(batch):
        write_file_path = DATA_SET + '/csv-for-' + attribute + '/' + path_list[i][:8] + '.' + data_type + '.csv'
        ...
        if cur_date not in path_list[i]:
            ...
            all_attributes_value.append(TYPE_NAME)
            all_attributes_value.append(TYPE_CODE)
            ...
            new_attributes_value.append(TYPE_NAME)
            new_attributes_value.append(TYPE_CODE)
